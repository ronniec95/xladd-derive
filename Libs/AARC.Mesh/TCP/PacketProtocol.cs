namespace AARC.Mesh.TCP
{
    ...
    /// <remarks>
    /// <para>
    /// Create one instance of this class for each incoming stream, and assign a handler to <see cref="MessageArrived"/>.
    /// As bytes arrive at the stream, pass them to <see cref="DataReceived"/>, which will invoke <see cref="MessageArrived"/> as necessary.
    /// If <see cref="DataReceived"/> raises <see cref="System.Net.ProtocolViolationException"/>, then the stream data should be considered invalid.
    /// After that point, no methods should be called on that <see cref="PacketProtocol"/> instance.
    /// This class uses a 4-byte signed integer length prefix, which allows for message sizes up to 2 GB.
    /// Keepalive messages are supported as messages with a length prefix of 0 and no message data.
    /// This is EXAMPLE CODE! It is not particularly efficient; in particular,
    /// if this class is rewritten so that a particular interface is used (e.g., Socket's IAsyncResult methods), some buffer copies become unnecessary and may be removed.
    /// </para>
    /// </remarks>
    public class PacketProtocol
    {
        ...
        /// </summary>
        public static byte[] WrapKeepaliveMessage() => BitConverter.GetBytes((int)0);
        ...
        public void DataReceived(byte[] data, int dataSize)
        {
        ...
        private void ReadCompleted(int count)
        {
            ...
            if (this.dataBuffer == null)
            {
                ...
                else
                {
                    ...
                    if (length == 0)
                    {
                        ...
                        this.MessageArrived?.Invoke(new byte[0]);
                    }
                    else
                    {
            ...
            else
            {
                ...
                else
                {
                    ...
                    this.MessageArrived?.Invoke(this.dataBuffer);
