...
class MeshServiceManager:
    ...
    def Process(self, clienttag: str, m: DiscoveryMessage):
            ...
            if state == DiscoveryStates.Register:
                if m.Port == 0:
                    while self.currentPort in self._usedPorts:
                        self.currentPort += 1
                    m = DiscoveryMessage(
                        m.State, self.currentPort, m.HostServer, None)
                else:
                ...
                logging.info("MSM Unknown state [%i : %s]" % (
                    m.State, m.HostServer))
        ...
        logging.info("MSM Bad Message [%s][%s : %s]" %
                     (clienttag, m.State, m.HostServer))
        return DiscoveryMessage(DiscoveryStates.Error, 0, m.HostServer, "{ ERROR: \"BAD MESSAGE\"}")
    ...
    def CreateGraphNodes(self):
        ...
        try:
            id = 0
            self._lock.acquire()
            id = self.CreateNodeConnectors(
                id, self.inputQs, True, channelNodes)
            id = self.CreateNodeConnectors(
                id, self.outputQs, False, channelNodes)
    ...
    def MeshNodesRoutesData(self):
        ...
        try:
            ...
            common = [k for k in (set(self.inputQs) & set(self.outputQs)) if (
                self.inputQs[k] - self.outputQs[k])]
                ...
                for oq in self.outputQs[queuename]:
                    for iq in self.inputQs[queuename]:
                        ...
                        if oq not in serverIds.keys():
                            ...
                            id += 1
                        if queuename not in serverIds.keys():
                            serverIds[queuename] = VisNode(
                                str(id), queuename, 'database')
                            id += 1

                        serverIds[queuename].connections.append(
                            serverIds[iq].id)
                        serverIds[oq].connections.append(
                            serverIds[queuename].id)
            routes += "["
            first = True
